...
const localSeed = () => ({
  shoppingItems: shoppingSeed.map((item) => ({
    ...item,
    state: shoppingStateToDatabase[item.state] || item.state,
    quantity: null,
    unit: '',
    note: item.detail || '',
  })),
  notices: noticeSeed.map((notice) => ({
    ...notice,
    id: String(notice.id),
    pinned: notice.priority === 'urgent',
    acknowledged: false,
    createdLabel: 'Recently',
  })),
  messages: messageSeed.map((message) => ({
    ...message,
    id: String(message.id),
    createdLabel: message.time,
  })),
...
export function AdultHouseholdProvider({ children }) {
  ...
  const loadLiveData = useCallback(async () => {
    ...
    try {
      ...
      const profileResult = actorIds.size
        ? await supabase.from('player_profiles').select('user_id, username').in('user_id', [...actorIds])
        : { data: [], error: null }
      if (profileResult.error) throw profileResult.error
      const profileById = Object.fromEntries((profileResult.data || []).map((profile) => [profile.user_id, profile.username]))
      ...
      setShoppingItems((shoppingResult.data || []).map((item) => ({ ...item, detail: detailForShoppingItem(item) })))
      setNotices((noticeResult.data || []).map((notice) => ({
        ...notice,
        author: notice.author_id === user.id ? 'You' : profileById[notice.author_id] || 'Housemate',
      ...
      setMessages((messageResult.data || []).map((message) => ({
        ...message,
        author: message.sender_id === user.id ? 'You' : profileById[message.sender_id] || 'Housemate',
      ...
      setActivity((activityResult.data || []).map((item) => ({
        ...item,
        member: item.actor_id === user.id ? 'You' : profileById[item.actor_id] || 'Household',
        action: item.summary,
      ...
      setHouseholdSettings({
        ...DEFAULT_SETTINGS,
        ...(settingsResult.data || {}),
        permissions: { ...DEFAULT_SETTINGS.permissions, ...(settingsResult.data?.permissions || {}) },
      })
  ...
  useEffect(() => {
    ...
    if (live) {
      ...
      const refreshSoon = () => {
        ...
        refreshTimer = window.setTimeout(loadLiveData, 120)
      }
      const houseId = activeHouse.id
      const channel = supabase
        .channel(`adult-household-${houseId}`)
    ...
    try {
      ...
      // A damaged demo cache should never prevent the app opening.
    }
  ...
  const addShoppingItem = async (input) => {
    const item = {
      ...
      state: shoppingStateToDatabase[input.state] || 'shopping_list',
    ...
    try {
      if (live) {
        ...
        setShoppingItems((current) => [{ ...item, id: makeId(), detail: detailForShoppingItem(item), created_at: new Date().toISOString() }, ...current])
      }
  ...
  const updateShoppingItem = async (id, patch) => {
    ...
    if (patch.state) databasePatch.state = shoppingStateToDatabase[patch.state] || patch.state
    try {
      if (live) {
        ...
        setShoppingItems((current) => current.map((item) => item.id === id ? { ...item, ...databasePatch, detail: detailForShoppingItem({ ...item, ...databasePatch }) } : item))
      }
  ...
  const addNotice = async (input) => {
    ...
    try {
      if (live) {
        const { error } = await supabase.from('household_notices').insert({ ...notice, household_id: activeHouse.id, author_id: user.id })
        if (error) throw error
        await loadLiveData()
      } else {
        setNotices((current) => [{ ...notice, id: makeId(), author: 'You', expires: formatExpiry(notice.expires_at), acknowledged: false, createdLabel: 'Now' }, ...current])
      }
  ...
  const acknowledgeNotice = async (id) => {
    try {
      if (live) {
        ...
        setNotices((current) => current.map((notice) => notice.id === id ? { ...notice, acknowledged: true } : notice))
      }
  ...
  const sendMessage = async (body, recipientId = null) => {
    const cleanBody = body.trim()
    if (!cleanBody || !householdSettings.messaging_enabled) return false
    try {
      if (live) {
        const { error } = await supabase.from('household_messages').insert({ household_id: activeHouse.id, sender_id: user.id, recipient_id: recipientId, body: cleanBody })
        if (error) throw error
        await loadLiveData()
      } else {
        setMessages((current) => [...current, { id: makeId(), author: 'You', body: cleanBody, time: 'Now', createdLabel: 'Now', mine: true, sender_id: user?.id || 'demo-user', recipient_id: recipientId }])
      }
      haptic('success')
      return true
    } catch (error) {
  ...
  const saveHouseholdSettings = async (patch) => {
    const next = {
      ...householdSettings,
      ...patch,
      permissions: { ...householdSettings.permissions, ...(patch.permissions || {}) },
    }
    try {
      if (live) {
        const payload = {
          ...
          notification_defaults: next.notification_defaults || {},
          contribution_mode: next.contribution_mode,
          messaging_enabled: next.messaging_enabled,
          direct_messages_enabled: next.direct_messages_enabled,
          notices_enabled: next.notices_enabled,
        }
        const { error } = await supabase.from('household_settings').upsert(payload, { onConflict: 'household_id' })
        if (error) throw error
        await loadLiveData()
      } else {
        setHouseholdSettings(next)
      }
  ...
  const value = useMemo(() => ({
    ...
    categories,
    membershipRole,
    canManageHousehold: membershipRole === 'owner' || membershipRole === 'admin',
    dataLoading,
    dataReady,
    dataError,
    refreshHouseholdData: loadLiveData,
    addShoppingItem,
    updateShoppingItem,
  ...
  }), [activity, categories, dataError, dataLoading, dataReady, householdSettings, loadLiveData, membershipRole, messages, notices, rooms, shoppingItems])
